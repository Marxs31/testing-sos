...
from .template import gui_template as temp
...
class first_frame(QWidget):
    ...
    def insert_emails(self):
        ...
        (login, password, smtp_server, smtp_port, imap_server, imap_port) = (
            load_credentials(get_path("sconf", "sconfig.json")))
    ...
    def send_email_status(self):
        ...
        if not recipient:
            ...
            self.alert_missing_text(self.recipient_info_label_2, "white", "red")
            missing_info = True


        if not subject:
            logger.error("Předmět emailu chybi.")
            # self.alert_missing_text(self.recipient_info_label_4, "white", "red")
            missing_info = False


        if not content:
            logger.error("Obsah emailu chybi.")
            self.alert_missing_text(self.email_content_label_2, "white", "red")
            missing_info = True
        ...
        if success == 1:
            ...
            self.enable_fields_after_sending()
    ...
    def send_email_success(self):
        ...
        QTimer.singleShot(5000, lambda: self.clear_content_entry(bg_default_color, 3))
    ...
    def clear_content_entry(self, default_color, idx):
        if idx == 3:
            ...
            self.recipient_info_label_2.setReadOnly(False)
            self.recipient_info_label_2.clear()
            self.recipient_info_label_2.setStyleSheet(temp.get_label_style() + f"background-color: {default_color};")

        elif idx == 1:
            self.recipient_info_label_4.setReadOnly(False)
            self.recipient_info_label_4.clear()
            self.recipient_info_label_4.setStyleSheet(temp.get_label_style() + f"background-color: {default_color};")

    def alert_missing_text(self, entry, default_color, select_color):

        if entry == self.recipient_info_label_4:
            entry.setStyleSheet(temp.get_label_style() + f"background-color: {select_color};")
            ...
            entry.setStyleSheet(temp.get_email_content_label() + f"background-color: {select_color};")
    ...
    def disable_fields_for_sending(self):
        ...
        # Nastav styl pro šedé pozadí
        self.recipient_info_label_2.setStyleSheet(temp.get_label_style() +"background-color: lightgray;")
        self.recipient_info_label_4.setStyleSheet(temp.get_label_style() +"background-color: lightgray;")
        self.email_content_label_2.setStyleSheet(temp.get_email_content_label() +"background-color: lightgray;")

    def enable_fields_after_sending(self):
        self.recipient_info_label_2.setReadOnly(False)
        self.recipient_info_label_4.setReadOnly(False)
        self.email_content_label_2.setReadOnly(False)

        self.recipient_info_label_2.setStyleSheet(temp.get_label_style() + f"background-color: white;")
        self.recipient_info_label_4.setStyleSheet(temp.get_label_style() + f"background-color: white;")
        self.email_content_label_2.setStyleSheet(temp.get_email_content_label() + f"background-color: white;")
